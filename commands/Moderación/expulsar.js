...
module.exports = class ECommand extends Command {
  constructor(client) {
    super(client, {
      ...
      examples: prefix => `\`${prefix}expulsar Sandessat Molestoso.\``,
      enabled: true,
      ownerOnly: false,
      guildOnly: false,
      aliases: ['kick'],
  ...
  async run(message, args, data) {
    try {
      if (!message.member.roles.has(this.client.config.servidor.roles.staff.departamento.comunidad)) return message.channel.send(':x: | No eres del Departamento Comunidad.');
      else
      if (!args[0]) {
