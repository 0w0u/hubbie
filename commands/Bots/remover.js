    ...
    try {
      if (!args[0]) {
        return message.channel.send(
          ':x: | Necesitas mencionar un bot para expulsar del Club de Bots.'
        );
      } else {
        let member =
        ...
        if (!member || !member.user.bot) {
          ...
          let embed = new (require('discord.js').RichEmbed)()
            ...
            .setDescription(
              'El bot ' +
                member.user.tag +
                ' ya no pertenece más al Club de bots'
            );
          message.channel.send({ embed });
          this.client.users.get(bot.info.ownerID).send({ embed });
          this.client.channels
            .get(this.client.config.servidor.categorias.bots.canales.playground)
            .send({ embed });
          member.kick(
            'Ya no pertenece al Club de Bots. Moderador/a responsable: ' +
              message.author.tag
          );
        }
      }
