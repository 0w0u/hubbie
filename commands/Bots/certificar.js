...
module.exports = class Bots extends Command {
	...
	async run(message, args, data) {
		try {
			if (!args[0]) {
				return message.channel.send(':x: | Necesitas especificar el bot a certificar.');
			} else {
				let bot = message.mentions.members.first() || message.guild.members.find(x => `${x.displayName}${x.user.tag}`.toLowerCase().includes(args[0].toLowerCase())) || message.guild.members.get(args[0]);
				if (!bot || !bot.user.bot) {
					return message.channel.send(':x: | Necesitas elegir a un bot');
				} else {
					let databot = await this.client.findOrCreateBot({ id: bot.user.id });
					if (!databot.accepted || !databot.invited || !databot.info) {
						return message.channel.send(':x: | Necesita ser un bot del Club de Bots.');
					} else {
						let embedDev = new (require('discord.js')).RichEmbed()
							.setColor(this.client.colors.hub)
							.setTitle('¡Bot certificado!')
							.setDescription('Tu bot ha sido certificado como "Confiable" dentro del Club de Bots.');
						this.client.users.get(databot.info.ownerID).send({ embed: embedDev });
