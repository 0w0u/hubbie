		...
		try {
			...
			if (!args[0]) {
				...
				let a = new embed()
					...
					.setDescription('Para ver la ayuda de un comando usa `' + (message.guild ? data.guild.prefix : 'h!') + 'ayuda <comando>`')
					.setFooter(message.author.tag, message.author.displayAvatarURL);
				categorias.forEach(cat => {
					let commandsize = comandos.filter(cmd => cmd.help.category === cat);
					a.addField(cat + ' • (' + commandsize.size + ')', commandsize.map(cmd => `\`${cmd.help.name}\` - ${cmd.help.description}`).join('\n'));
				});
				...
				if (message.member.hasPermission("MANAGE_GUILD")) {
					...
					categorias.forEach(cat => {
						let commandsize = comandos.filter(cmd => cmd.help.category === cat);
						c.addField(cat + ' • (' + commandsize.size + ')', commandsize.map(cmd => `\`${cmd.help.name}\` - ${cmd.help.description}`).join('\n'));
					});
				...
				if (!comandos.has(args[0].toLowerCase()) && !comandos.has(aliases.get(args[0].toLowerCase()))) {
					...
					let e = new embed()
						...
						.addField('Alias(es):', h.config.aliases.map(a => `\`${a}\``).join('`, `'));
