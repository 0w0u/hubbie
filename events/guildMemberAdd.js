      ...
      if (member.user.bot) {
        ...
        if (bot.invited) {
          ...
          this.client.channels
            ...
            .send('[Bot] **' + member.user.tag + '** ha sido invitado y requiere de aprobación');
        } else {
        ...
        this.client.channels.get(servidor.categorias.comunidad.canales.global).send(member.user.toString(), { embed: embed });
        this.client.channels.get(servidor.categorias.staff.canales.logs).send('[Común] **' + member.user.tag + '** ha entrado al servidor')
      }
    } catch (e) {
      console.error(e);
    }
